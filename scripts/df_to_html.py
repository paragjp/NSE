...
def df_to_html():
    ...
    write_to_html_file1(df, 'CE/PE Base File', 'c:\\NSE\\outputs\\CEPEBasefile.html')

    df = pd.read_excel('C:\\NSE\\outputs\\CEPELog.xlsx', index_col=None)
    df.columns = \
        df.columns.str.strip().str.lower().str.replace(' ', '_').str.replace('(', '').str.replace(')', '')
    write_to_html_file1(df, 'CE/PE Log File', 'c:\\NSE\\outputs\\CEPELogfile.html')
